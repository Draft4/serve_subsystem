...
`ServePlannerNode._robot_callback`：缓存定位；`_plan`：新鲜度与模型规划；`ModelService`：快照校验和参数推断；`launcher_strategy/`：策略实现。
...
| `ModelService.metadata` | manifest 与快照 → 模型元数据 | 缺文件或哈希不符抛 `ModelError` |
