...
| --- | --- | --- | --- | --- | --- |
| [`tennis_serve_aim`](../src/tennis_serve_aim/wiki/README.md) | `03_planning_control` | `serve_planning_ws` | 路遥（开发规范） | 机器人当前位置的发球目标朝向规划。 | 已实现；现场联调未验证 |

层级为 `03_planning_control`；边界：根据当前定位与目标计算 yaw；不生成发球路径或底盘轨迹。
...
输入输出细节以每个包的 [interfaces.md](../src/tennis_serve_aim/wiki/interfaces.md)
